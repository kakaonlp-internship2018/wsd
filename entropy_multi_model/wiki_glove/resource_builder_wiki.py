...
def make_glove_bin():
    ...
    with open(GLOVE_TXT, 'r') as fr_txt, open(GLOVE, 'wb') as fw_bin:
        ...
        for line in fr_txt:
            ...
            count = count + 1
            if count % 100000 == 0:
                LOGGER.info(str(count) + " word vectors loaded")
        LOGGER.info("Loading embedding vectors done, " + str(count))
...
def build_svm_for_difficult_word():
    ...
    #open(SVM_DIC, 'rb') as fr_svm_dic:
            ...
            for index, token in enumerate(new_tokens):
                if TKN_PTN.match(token):
                    ...
                    if ent_dic[key] >= ENTROPY_THRESHOLD: #and svm_dic.get(key) is None:
                        ...
                        tokens_for_emb = re.split('[ ]', re.sub(r'__[\d][\d]/[A-Z]+', '', line)) # wiki version(without pos tag)
                        ...
                        count = count + 1
                        if count % 100000 == 0:
                            LOGGER.info(str(count) + " tokens finished building training data")

        LOGGER.info("Building training data done, " + str(count))
        ...
        for key, training_data in train_dic.items():
            ...
            count = count + 1
            if count % 1000 == 0:
                LOGGER.info(str(count) + " words finished training svm model")

        LOGGER.info("Training svm model done, " + str(count))
